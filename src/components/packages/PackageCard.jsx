...
export default function PackageCard({ pkg, onAddToCart }) {
  ...
  return (
    <div className="bg-white border border-gray-100 rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-all duration-300 flex flex-col group h-full">
      
      {/* Package Image Banner */}
      <div className="h-52 bg-zinc-50 relative overflow-hidden shrink-0">
         {pkg.image ? (
           <img 
             src={pkg.image} 
             alt={pkg.name} 
      ...
      <div className="p-6 flex-grow flex flex-col justify-between space-y-5">
        ...
        <div className="space-y-4">
          ...
          <div className="flex justify-between items-start gap-2">
            <div className="space-y-0.5">
              <h3 className="font-serif text-lg font-bold text-zinc-850 tracking-wide leading-tight">{pkg.name}</h3>
              <p className="font-sans text-[11px] uppercase tracking-widest text-emerald-700/80 font-semibold">Luxury Set</p>
            </div>
            <span className="font-sans text-sm font-bold text-emerald-800 bg-emerald-50 px-2.5 py-1 rounded-md border border-emerald-100 shrink-0">
              {pkg.price} BDT
            </span>
          </div>
              ...
              <div className="space-y-3">
                ...
                <div className="flex gap-2">
                  <select
                    ...
                    className="flex-1 bg-gray-50 border border-gray-200/80 rounded-lg px-3 py-2 text-xs focus:outline-none focus:border-emerald-700 focus:ring-2 focus:ring-emerald-700/5 focus:bg-white transition-all font-sans font-medium text-zinc-800 cursor-pointer appearance-none shadow-3xs"
                    style={{
                  ...
                  <button
                    ...
                    className="bg-emerald-700 hover:bg-emerald-800 text-white px-3 rounded-lg transition-all duration-200 flex items-center justify-center shadow-3xs active:scale-95 shrink-0"
                  >
                ...
                <div className="bg-gray-50/40 p-3 rounded-xl border border-gray-100 space-y-2 shadow-3xs">
                  <div className="flex justify-between items-center text-[10px] font-bold tracking-wider uppercase text-gray-400 font-sans">
                    ...
                    <span className={`text-[10px] px-2 py-0.5 rounded-md font-extrabold ${selectedAtors.length === 5 ? "bg-emerald-50 text-emerald-600 border border-emerald-100" : "bg-zinc-100 text-zinc-600 border border-gray-100"}`}>
                  ...
                  {selectedAtors.length === 0 ? (
                    ...
                    <ul className="text-[11px] text-zinc-800 font-sans space-y-1 max-h-32 overflow-y-auto pr-1">
                      {selectedAtors.map((item, i) => (
                        <li key={i} className="flex items-center justify-between bg-white px-2.5 py-1.5 rounded-lg border border-gray-100 shadow-3xs">
                          <span className="font-semibold text-gray-700 truncate mr-2">
                            <span className="text-emerald-700/60 mr-1 font-mono">0{i + 1}.</span> {item}
                          </span>
                          <button 
                            type="button" 
        ...
        <div className="pt-2">
          <button
            ...
            className={`w-full font-sans text-[11px] uppercase tracking-widest font-bold py-3.5 rounded-lg transition-all duration-300 flex items-center justify-center space-x-2 shadow-xs cursor-pointer ${
